...
from bion_rectangle.utils.plt_network import draw_bipartite
...
def imshow_discrete(x, shade=None,
    ...
    for i_color in range(n_color):
        ...
        c += incl * shade * np.float32(vec_on(colors[i_color], 2, 3)) \
            + incl * (1. - shade) * np.float32(vec_on(color_shade, 2, 3))
...
def multiheatmap(
    ...
    if len(shape) == 4:
        ...
        hs, array_subaxs = np.vectorize(
            ...
            a=arrayobj(a, ndim_objarray=2),
            parent=axs.axs,
            kw_imshow=kw_imshow,
            inches_per_cell=inches_per_cell,
            wspace=arrayobj1d([wspace[1:]]),
            hspace=arrayobj1d([hspace[1:]]),
        )
    hs = cell2mat(hs, dtype=object)
    return hs, (axs, array_subaxs)
...
def plot_binned_ch(x0, ch, n_bin=9, **kw):
    ix, x = quantilize(x0, n_quantile=n_bin, return_summary=True)
...
class LatexDoc(ltx.Document, ContextManager):
    def __init__(
        self,
        *args,
        file_out=None,
...
def imshow_confusion(
    ...
    if kind == 'p_best_given_true':
        p_plot_fit_sim = sumto1(n_best_fit_by_sim, 0)
    elif kind == 'p_true_given_best':
        p_plot_fit_sim = sumto1(n_best_fit_by_sim, 1)
    else:
        raise ValueError()
...
def count_best_model(best_model_sim):
    ...
    n_best_fit_by_sim = npg.aggregate(
        np.reshape(
            np.broadcast_arrays(
                permute2en(best_model_sim)[:, None],
                # [sim, fit, subj]
