...
from bion_rectangle.utils.obj_torch import PyTorchObjective
...
from bion_rectangle.utils.cacheutil import (
    mkdir4file, 
    is_keyboard_interrupt, 
)
from bion_rectangle.utils.localfile import LocalFile
...
class OverriddenParameter(nn.Module):
    ...
    def _load_from_state_dict(
        ...
        if self.skip_loading_lbub:
            ...
            if lb_name in state_dict:
                self.lb = npt_tensor(state_dict.pop(lb_name))
            if ub_name in state_dict:
                self.ub = npt_tensor(state_dict.pop(ub_name))

        if data_name in state_dict:
            state_dict[param_name] = self.data2param(
                state_dict.pop(data_name).detach().clone().to(get_device()))
        elif param_name in state_dict:
            state_dict[param_name] = npt_tensor(state_dict[param_name])
...
class BoundedParameter(OverriddenParameter):
    def __init__(self, data, lb=0., ub=1.,
        ...
        # self.lb = None if lb == -np.inf or lb is None else npt_tensor(lb)
        # self.ub = None if ub == np.inf or ub is None else npt_tensor(ub)
    ...
    def data2param(self, data) -> torch.Tensor:
        ...
        lb = npt_tensor(self.lb)
        ub = npt_tensor(self.ub)
        data = npt_tensor(enforce_float_tensor(data))
    ...
    def param2data(self, param):
        ...
        def tensor1(v):
            return npt_tensor(v).to(param.device)
        if lb is not None:
            lb = tensor1(lb)
        if ub is not None:
            ub = tensor1(ub)
    ...
    def get_n_free_param(self) -> int:
        ...
        if not self._param.requires_grad:
            ...
            lb = npt_tensor(-np.inf) if self.lb is None else self.lb
            ub = npt_tensor(np.inf) if self.ub is None else self.ub
            if self._fix_subset:
    ...
    def __init__(self, prob, probdim=0, requires_grad=True, randomize=False,
        ...
        if randomize:
            ...
            prob = np.swapaxes(npy(torch.distributions.Dirichlet(
                npt_tensor(np.ones_like(prob1))
            ).sample()), probdim, -1)
        prob = npt_tensor(enforce_float_tensor(prob))
...
class BoundedModule(nn.Module):
    ...
    def register_bounded_parameter(self, name, data, lb=0., ub=1.):
        lb = npt_tensor(lb, min_ndim=0)
        ub = npt_tensor(ub, min_ndim=0)
        data = enforce_float_tensor(data)
    ...
    def plot_params(
        ...
        box_off(['top', 'right', 'bottom'])
        detach_axis('x', amin=0, amax=1)
        detach_axis('y', amin=0, amax=n - 1)
    ...
    def get_named_bounded_params(
        ...
        for name, param in d.items():
            ...
            if param.lb is None:
                l0 = npt_tensor([-np.inf]).expand_as(param.v).flatten()
            else:
                l0 = npt_tensor(param.lb).expand_as(param.v).flatten()
            if param.ub is None:
                u0 = npt_tensor([np.inf]).expand_as(param.v).flatten()
            else:
                u0 = npt_tensor(param.ub).expand_as(param.v).flatten()
    ...
    def param_vec2dict(self, param_vec, suffix: str) -> odict:
        ...
        if not torch.is_tensor(param_vec):
            param_vec = npt_tensor(param_vec)
    ...
    def set_requires_grad_(self, requires_grad: bool):
        ...
        set_requires_grad(self, requires_grad)
...
def enforce_float_tensor(v: Union[torch.Tensor, np.ndarray], device=None
    ...
    if not torch.is_tensor(v):
        return npt_tensor(v, device=device)
...
def detach_seq(seq):
    return fun_seq(lambda v: npt_tensor(npy(v)), seq)


def is_nan_by_batch(v: torch.Tensor):
    return (
        torch.isnan(v.reshape(v.shape[0], -1)).any(-1)
        if v.shape[0] > 0 else zeros([0], dtype=bool)
    )
...
def optimize(
        ...
        for epoch in range(max([max_epoch, 1])):
            ...
            for i_fold in range(n_fold_valid):
                ...
                if n_fold_valid == 1:
                    ...
                    loss_valid1 = npt_tensor(npy(loss_train1))
            ...
            if loss_valid < best_loss_valid:
                ...
                best_loss_valid = npt_tensor(npy(loss_valid))
                best_state = model.state_dict()
    ...
    for mode in ['train_valid', 'valid', 'test', 'all']:
        ...
        d.update({
            ...
            'loss_' + mode: npt_tensor(npy(loss))
        })
